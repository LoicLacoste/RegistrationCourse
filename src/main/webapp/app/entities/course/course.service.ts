...
export class CourseService {
  ...
  protected convertDateFromClient(course: ICourse): ICourse {
    const copy: ICourse = Object.assign({}, course, {
      dateCourse: course.dateCourse && course.dateCourse.isValid() ? course.dateCourse.toJSON() : undefined
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateCourse = res.body.dateCourse ? moment(res.body.dateCourse) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((course: ICourse) => {
        course.dateCourse = course.dateCourse ? moment(course.dateCourse) : undefined;
      });
    }
    return res;
  }
